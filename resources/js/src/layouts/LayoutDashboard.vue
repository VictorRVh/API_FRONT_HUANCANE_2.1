...
<template>
  <div class="flex flex-col lg:flex-row w-full h-screen bg-white dark:bg-gray-800 overflow-hidden">
    <!-- Sidebar Header -->
    <header v-if="sidebarOpen || isLargeScreen" class="w-full lg:w-[15%] bg-plomoClaro dark:bg-gray-800 h-full sticky top-0 shadow-lg z-10">
    ...
    <main class="flex-1 h-full bg-gray-100 dark:bg-gray-900 overflow-hidden relative">
      ...
      <button v-if="!isLargeScreen" @click="sidebarOpen = !sidebarOpen" class="fixed top-1/2 -translate-y-1/2 left-0 z-20 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 shadow hover:ring-2 hover:ring-gray-500 dark:hover:ring-gray-300 transition duration-150 rounded-lg p-1 flex items-center justify-center">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-2 h-24 text-gray-500 dark:text-gray-300">
          <path stroke-linecap="round" stroke-linejoin="round" :d="sidebarOpen ? 'M8.25 19.5L15.75 12 8.25 4.5' : 'M15.75 19.5L8.25 12 15.75 4.5'" />
        </svg>
      </button>
      ...
      <div class="flex items-center justify-between p-4 bg-blancoPuro dark:bg-gray-800 border-b border-gray-300 dark:border-gray-600 mx-auto rounded-lg w-[99%] mt-2">
        ...
        <div class="flex items-center space-x-4 relative">
          ...
          <div class="w-10 h-10 rounded-full flex items-center justify-center bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-300 cursor-pointer" @click.stop="toggleData">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
              <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
            </svg>
          </div>
          <div v-if="mostrarData" id="data-dropdown" class="absolute top-12 right-0 w-64 bg-white dark:bg-gray-700 rounded-lg shadow-lg p-4 z-50">
            <Data />
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center justify-start p-4 bg-blancoPuro dark:bg-gray-800 border-b border-gray-300 dark:border-gray-600 mx-auto rounded-lg w-[99%] mt-1">
        <button @click="goBack" class="flex items-center justify-center w-8 h-6 rounded-full border border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-300 hover:ring-2 hover:ring-gray-500 dark:hover:ring-gray-300 transition duration-150">
