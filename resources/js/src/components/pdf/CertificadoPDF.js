...
export function generateCertificate(data, certificado) {
  ...
  doc.text(certificado.apellidos_nombres || "NOMBRE DEL BENEFICIARIO", 60, posY + 60);
  ...
  doc.setFont("times", "bold");
  doc.text(certificado.especialidad.toUpperCase() || "NOMBRE DEL MÓDULO", 148.5, posY + 80, { align: "center" });

  doc.setFont("times", "italic");
  doc.text("Correspondiente al Programa de Estudios:", 20, posY + 90);

  doc.setFont("times", "bold");
  doc.text(certificado.programa.toUpperCase() || "NOMBRE DE ESPECIALIDAD", 148.5, posY + 100, { align: "center" });
  ...
  const formattedDateRange = formatDateRangeFromSlash(certificado.fecha_inicio, certificado.fecha_fin);

  let sumCreditos=0;
  let sumHoras = 0;
  certificado.unidades_didacticas.map((unidad)=>{
     sumCreditos += unidad.credito;
     sumHoras += unidad.hora;
  })
  sumHoras += certificado?.experiencias_formativas[0]?.horas_exp;
  sumCreditos += certificado?.experiencias_formativas[0]?.creditos_exp;
  doc.setFont("times", "italic");
  doc.text(
  ...
  const rows = certificado?.unidades_didacticas.map((unidad, index) => {
    if (index === 0) {
      return [
  ...
  doc.autoTable({
    didDrawCell: function (data) {
      if (data.column.index === 0 && data.cell.raw && data.cell.raw.content) {
        ...
        const text = certificado?.unidad_competencia;
